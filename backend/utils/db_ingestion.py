...
def ingest_documents():
    ...
    try:
        ...
        force_ingestion = os.getenv("FORCE_DOCUMENT_INGESTION", "false").strip().lower() in {"1", "true", "yes", "on"}
        existing_report_count = db.query(PatientReport).count()
        ...
        if not force_ingestion and existing_report_count > 0 and existing_chunk_count > 0:
            print("Document data already exists. Skipping document ingestion.")
            return
