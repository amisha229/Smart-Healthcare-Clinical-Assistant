...
def setup_common(monkeypatch, db, embedded=None):
    ...
    fake_embeddings = FakeEmbeddings(vector=embedded if embedded is not None else [0.1, 0.2])
    monkeypatch.setattr(rs, "_get_embeddings", lambda: fake_embeddings)
...
def test_embed_query_called(monkeypatch, query):
    ...
    monkeypatch.setattr(rs, "_get_embeddings", lambda: SpyEmbeddings())
...
def test_embed_exception_propagates_before_db_creation(monkeypatch):
    ...
    monkeypatch.setattr(rs, "_get_embeddings", lambda: FailingEmbeddings())
